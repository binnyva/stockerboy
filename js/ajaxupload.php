			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					if($ext==end($file_ext)){
						...
						$NUM = time();
						$front_name = substr($file_ext[0], 0, 5);
						...
						if(!file_exists($save)){
							...
							if($maxH == null){
								...
								if($fheight == 0 || $fwidth == 0 || $height_orig == 0 || $width_orig == 0){
									die("FATAL ERROR REPORT ERROR CODE [add-pic-line-67-orig] to <a href='#'>AT WEB RESULTS</a>");
								}
								if($fheight < 45){
									$blank_height = 45;
...
<?php
	...
	{
		//include_once "db.php";
		//$image = base64_encode(file_get_contents("../uploads/images/$upload_image"));	
		//$query = "INSERT INTO image_questions (image) VALUES('$image')";
		//mysql_query($query);
		//$last_id = mysql_insert_id();
		//echo $last_id;
		//echo '<img src="/externals/showimage.php?id='.$last_id.'"  style="border:none;" />';		
		// echo '<img src="../uploads/tulips_1291439132.jpg" />';
